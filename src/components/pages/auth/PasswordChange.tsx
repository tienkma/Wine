...
const PasswordChange = () => {
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      retypePassword: Yup.string()
        ...
        .oneOf([Yup.ref("newPassword"), null], "Retype password must match"),
  ...
  const handleSubmit = async (value: any) => {
    ...
    try {
        const data = await Requests.post(`${URL_REQUEST}/api/v1/user/${id}`, {password, newPassword});
        if(data?.error){
